...
def load_cloud_settings():
    ...
    try:
        # worksheet="Settings" must match your Google Sheet tab name exactly
        df = conn.read(spreadsheet=SHEET_URL, worksheet="Settings")
        if not df.empty:
            # Create a dictionary from the Key and Value columns
            stored_settings = dict(zip(df['Key'].astype(str), df['Value'].astype(str)))
            # Ensure essential keys exist, otherwise use defaults
            if "limit" not in stored_settings: stored_settings["limit"] = "1000.0"
            if "start_date" not in stored_settings: stored_settings["start_date"] = str(get_today_ist())
            if "pin_hash" not in stored_settings: stored_settings["pin_hash"] = DEFAULT_PIN_HASH
            return stored_settings
    except Exception as e:
        st.error(f"Cloud Settings Error: {e}")
    
    # Absolute fallback if sheet is unreadable
    return {"limit": "1000.0", "start_date": str(get_today_ist()), "pin_hash": DEFAULT_PIN_HASH}

# --- INITIALIZE SESSION STATE ---
# This ensures settings are loaded ONCE and persist during the session
if "settings" not in st.session_state:
    st.session_state.settings = load_cloud_settings()

# Use a local reference for easier coding
...
def check_password():
    ...
    if not st.session_state.authenticated:
        st.title("🔒 App Locked (Emergency Mode)")
        st.caption("Enter 1175 to reset your cloud settings.")
        
        pin = st.text_input("4-Digit PIN", type="password")
        
        if st.button("Unlock", type="primary"):
            # This ignores the Google Sheet and looks only at this text:
            if pin == "1175":
